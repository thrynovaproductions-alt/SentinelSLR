import streamlit as st
from google import genai
from PIL import Image, ImageEnhance
import sqlite3
import pandas as pd
from datetime import datetime
import json
import os
import io

# --- 1. SETUP & CONFIGURATION ---
API_KEY = st.secrets["GEMINI_API_KEY"]
client = genai.Client(api_key=API_KEY)
DB_FILE = "sentinel_slr.db"
CONFIG_FILE = "sentinel_config.json"

def load_config():
    if not os.path.exists(CONFIG_FILE):
        return {"version": 1.9, "rule_stats": {
            "Avoid chasing vertical moves.": {"wins": 0, "losses": 0},
            "Check RSI for 70+ levels.": {"wins": 0, "losses": 0}
        }}
    with open(CONFIG_FILE, 'r') as f: return json.load(f)

# --- 2. AUTOMATED RECOGNITION ENGINE ---
def automated_audit(current_price):
    """The 'Look-Back': Scans for pending trades and closes them automatically."""
    conn = sqlite3.connect(DB_FILE)
    pending = pd.read_sql_query("SELECT * FROM slr_log WHERE outcome IS NULL", conn)
    config = load_config()
    updated = False

    for _, trade in pending.iterrows():
        status = None
        # Recognition Logic: App deduces the result based on the new chart's price
        if current_price >= trade['target_price']:
            status = 'Win ✅'
            config['rule_stats'][trade['rule_applied']]['wins'] += 1
            updated = True
        elif current_price <= trade['stop_price']:
            status = 'Loss ❌'
            config['rule_stats'][trade['rule_applied']]['losses'] += 1
            updated = True
            # AI Reflection: Learning why the previous verdict failed
            p = f"Reflect: Rule '{trade['rule_applied']}' failed. Price hit {current_price} after predicting target {trade['target_price']}. What went wrong?"
            res = client.models.generate_content(model="gemini-2.0-flash", contents=[p])
            conn.execute("UPDATE slr_log SET reflection_text = ? WHERE id = ?", (res.text, trade['id']))

        if status:
            conn.execute("UPDATE slr_log SET outcome = ? WHERE id = ?", (status, trade['id']))
    
    conn.commit()
    conn.close()
    if updated:
        with open(CONFIG_FILE, 'w') as f: json.dump(config, f)

# --- 3. ANALYST ENGINE (AUDIT -> PREDICT) ---
def process_chart(img_file, best_rule):
    raw_img = Image.open(img_file)
    processed_img = ImageEnhance.Contrast(raw_img).enhance(1.6) # Enhance for price OCR
    
    # Prompt requires the AI to find current price for the audit
    prompt = f"""Rule: {best_rule}. 
    Analyze this chart and extract the CURRENT PRICE exactly.
    Return ONLY JSON: 
    {{"verdict": "BUY/SELL", "price": float, "target": float, "stop": float, "logic": "str"}}"""
    
    response = client.models.generate_content(model="gemini-2.0-flash", contents=[prompt, processed_img])
    
    try:
        data = json.loads(response.text)
        # STEP 1: AUTO-AUDIT (Audit the past using data from the new chart)
        automated_audit(data['price'])
        
        # STEP 2: PREDICT (Log the current chart as the new pending trade)
        conn = sqlite3.connect(DB_FILE)
        conn.execute("INSERT INTO slr_log (timestamp, verdict_text, rule_applied, entry_price, target_price, stop_price) VALUES (?, ?, ?, ?, ?, ?)",
                     (datetime.now().strftime("%Y-%m-%d %H:%M"), data['logic'], best_rule, data['price'], data['target'], data['stop']))
        conn.commit(); conn.close()
        return data
    except Exception as e:
        return {"error": f"Parse Error: {e}", "raw": response.text}

# --- 4. LAYOUT & UI ---
st.set_page_config(page_title="🛡️ Sentinel", layout="wide")

# Initialize Database
conn = sqlite3.connect(DB_FILE)
conn.execute('''CREATE TABLE IF NOT EXISTS slr_log 
             (id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp DATETIME, verdict_text TEXT, 
             outcome TEXT, rule_applied TEXT, entry_price REAL, target_price REAL, 
             stop_price REAL, reflection_text TEXT)''')
conn.close()

with st.sidebar:
    st.header("🧬 Strategy Lab")
    config = load_config()
    st.write(f"Version: v{config['version']}")
    if st.button("Download DB Backup"):
        if os.path.exists(DB_FILE):
            with open(DB_FILE, "rb") as f:
                st.download_button("📥 Click to Download", f, "sentinel.db")

st.title("🛡️ Sentinel Autonomous Intelligence")
tab1, tab2 = st.tabs(["📸 Analyst Scanner", "📊 Audit Log"])

with tab1:
    files = st.file_uploader("Upload New Charts", type=["jpg", "png", "jpeg"], accept_multiple_files=True)
    if files and st.button("🚀 Run Auto-Audit & Predict"):
        best_rule = max(config['rule_stats'], key=lambda x: (config['rule_stats'][x]['wins']+1)/(config['rule_stats'][x]['wins']+config['rule_stats'][x]['losses']+1))
        for f in files:
            result = process_chart(f, best_rule)
            st.write(result)

with tab2:
    st.dataframe(pd.read_sql_query("SELECT * FROM slr_log ORDER BY id DESC", sqlite3.connect(DB_FILE)))
